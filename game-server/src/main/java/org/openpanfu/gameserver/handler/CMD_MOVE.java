...
public class CMD_MOVE implements IHandler {
	...
	public void handlePacket(PanfuPacket packet, User sender) {
		int fromX = sender.getX();
		int fromY = sender.getY();
		if (packet.getParameterCount() >= 5) {
			fromX = packet.readInt();
			fromY = packet.readInt();
			sender.setX(fromX);
			sender.setY(fromY);
		}
		...
		case MOVEMENT_TYPE_WALK:
			...
			if (sender.getTimeUntilWalkComplete() - System.currentTimeMillis() > 0) {
				...
				sender.setTimeUntilWalkComplete(System.currentTimeMillis() + calcuTime(fromX, fromY, toX, toY) + msLeft);
			}
			sender.move(toX, toY, calcuTime(fromX, fromY, toX, toY) + msLeft, type);
			break;
		default:
